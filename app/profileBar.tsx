...
export default function ProfileBar(props: IProfileBarProps) {
  ...
  return (
    <div className="flex flex-row justify-between px-8 py-4 text-xl bg-slate-400 w-screen items-center">
      <p>
        {!!user
          ? `You are signed in as ${user.displayName} (${user.userPrincipalName})`
          : 'Please sign in'}
      </p>
      <button
        onClick={clickAction}
        className="bg-gray-500 hover:bg-slate-300 rounded border-slate-500 border-2 p-2 flex flex-row"
      >
